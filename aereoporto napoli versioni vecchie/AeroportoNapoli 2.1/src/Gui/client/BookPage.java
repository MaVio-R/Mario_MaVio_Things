...
public class BookPage extends BasePanel implements Resettable{
    ...
    public BookPage(JPanel container,PanelController panelController) {
        ...
        initComponents();
 // Aggiungi un ListSelectionListener
booktable.getSelectionModel().addListSelectionListener(event -> {
    if (!event.getValueIsAdjusting()) {
        ...
        if (selectedRow != -1) {
            ...
            // Recupera i valori dalle colonne della riga selezionata
            String flightNumber = (String) model.getValueAt(selectedRow, 0);
            String departure = (String) model.getValueAt(selectedRow, 3); // Partenza
            String arrival = (String) model.getValueAt(selectedRow, 4);   // Arrivo
            String date = (String) model.getValueAt(selectedRow, 1);      // Data
            String time = (String) model.getValueAt(selectedRow, 2);      // Orario
            String company = (String) model.getValueAt(selectedRow, 5);   // Compagnia

            // Conversione della stringa data e orario in java.sql.Date e java.sql.Time
            try {
                java.sql.Date sqlDate = java.sql.Date.valueOf(date); // Converte una stringa in formato "yyyy-MM-dd"
                java.sql.Time sqlTime = java.sql.Time.valueOf(time + ":00"); // Specifica anche i secondi

                // Assegna i valori alle caselle di testo
                FlightNumber.setText(flightNumber);
                Departure.setText(departure);
                Arrival.setText(arrival);
                Date.setText(date);
                DepartureTime.setText(time);
                FlightCompany.setText(company);

                // Crea un'istanza di Flight e assegnala a 'selectedFlight'
                selectedFlight = new Flight(0, flightNumber, company, departure, arrival, sqlDate, sqlTime, null, null, "SCHEDULED");
            } catch (IllegalArgumentException e) {
                JOptionPane.showMessageDialog(this, "Errore nella conversione del formato di data o ora: " + e.getMessage(),
                        "Errore di Conversione", JOptionPane.ERROR_MESSAGE);
            }
        }
    }
});
   Controller.loadBookableFlights(booktable);
    ...
    private void initComponents() {
        ...
        booktable.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Numero Volo", "Data", "Ora", "Partenza", "Destinazione"
            }
        ) {
    ...
    String firstName = FlightNumber.getText().trim();
    String lastName = FlightCompany.getText().trim();
    ...
    // Verifica che un volo sia stato selezionato
    if (selectedFlight == null) {
        JOptionPane.showMessageDialog(
            this,
            "Selezionare un volo dalla tabella prima di prenotare.",
            "Errore",
            JOptionPane.ERROR_MESSAGE
        );
        return;
    ...
    try {
        boolean success = Controller.bookFlight(
            AeroportoNapoli.LoggedUser.getUserId(), // Ottieni ID dell'utente loggato
            selectedFlight.getId(),                 // Ottieni ID del volo selezionato
            firstName,
            lastName
        );
        ...
        if (success) {
            ...
            resetPanel(); // Resetta i campi e la selezione
        } else {
            JOptionPane.showMessageDialog(
                this,
    ...
    public void resetPanel() {
        // Ricarica i voli prenotabili nella tabella
        Controller.loadBookableFlights(booktable);
        ...
        // Deseleziona qualsiasi riga nella tabella
